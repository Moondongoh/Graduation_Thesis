...
def predict_single_file(model, file_path, max_len, device):
    ...
    tensor_feature = (
        torch.tensor(padded_feature, dtype=torch.float32)
        .unsqueeze(0)
        .unsqueeze(0)
        .to(device)
    )
...
if __name__ == "__main__":
    ...
    TEST_DATA_PATH = r"D:\MDO\heartbeat\1_New_HB_0818\2D_CNN\change_csv\validation_data"
    TEST_CSV_PATH = r"D:\MDO\heartbeat\1_New_HB_0818\2D_CNN\change_csv\validation_data\REFERENCE2.csv"
    ...
    try:
        ...
        true_labels = {
            os.path.splitext(str(row[0]))[0]: row[1]
            ...
            if filename.endswith(".npy"):
                ...
                if true_labels:
                    ...
                    if base_filename in true_labels:
                        ...
                        print(
                            f"파일: {filename}, 예측: {predicted_class_label}, 정답: {true_label}",
                            end="",
                        )
                        ...
                        print(
                            f"파일: {filename}, 예측: {predicted_class_label} (정답 라벨 없음)"
                        )
                else:
...
# if __name__ == '__main__':
#     # --- 설정값 ---
#     MODEL_PATH = r'D:\MDO\heartbeat\1_New_HB_0818\2D_CNN\model\4binary_audio_cnn_model.pth' # 훈련 스크립트에서 저장한 모델 파일
#     TEST_DATA_PATH = r'D:\MDO\heartbeat\1_New_HB_0818\2D_CNN\change_csv\validation_data'
