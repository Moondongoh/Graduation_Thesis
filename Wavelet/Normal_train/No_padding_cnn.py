        ...
        for i, path in enumerate(self.data_file_paths):
            try:
                ...
                if len(y_data) >= self.max_sequence_length:
                    y_trimmed = y_data[: self.max_sequence_length]
                    self.cached_data[i] = torch.tensor(
                        y_trimmed.reshape(1, -1), dtype=torch.float32
                    )
                else:
                    print(
                        f"  건너뜸: {os.path.basename(path)} (길이 부족: {len(y_data)} samples)"
                    )
                    self.cached_data[i] = None

            except Exception as e:
                print(
                    f"  데이터셋 로드 오류: '{os.path.basename(path)}' - {e}. 해당 인덱스 건너뜸."
                )
                self.cached_data[i] = None
