...
def extract_wavelet_features(signal, wavelet="db4", level=5, segment_length=512):
    ...
    for i in range(num_segments):
        ...
        segment_features = []
        # cA5, cD5, cD4, cD3, cD2, cD1 계수들로부터 특징을 추출합니다.
        for c in coeffs:
...
# 라벨 파일 로드
try:
    # CSV 파일을 읽을 때, 파일 확장자를 제거하고 라벨을 저장합니다.
...
for filename in tqdm(os.listdir(input_dir)):
    if filename.endswith(".wav"):
        ...
        try:
            signal, sr = sf.read(filepath)  # .wav 파일 로드
            features = extract_wavelet_features(signal)  # (세그먼트 수, 30)

            # 파일명(확장자 제외)을 키로 사용하여 라벨 가져오기
