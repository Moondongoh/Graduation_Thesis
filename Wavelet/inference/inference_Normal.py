...
def load_labels_and_data_paths(data_base_path, label_file_path):
    ...
    try:
        ...
        for index, row in df_labels.iterrows():
            # 🚨 중요: 모델이 학습된 데이터의 파일 확장자와 일치시켜야 합니다! 🚨
            # 만약 모델이 원본 1D 데이터를 사용했다면 '_data.txt'
            # 만약 모델이 웨이블릿 변환 데이터를 사용했다면 '_wavelet.txt'
            txt_filename = (
                f"{row['filename_prefix']}_data.txt"  # 현재 원본 1D TXT 데이터를 로드
            )
            # txt_filename = f"{row['filename_prefix']}_wavelet.txt" # 만약 모델이 웨이블릿 데이터로 학습되었다면 이 줄을 사용
...
def load_data_for_cnn(data_file_paths, max_sequence_length):
    ...
    for i, path in enumerate(data_file_paths):
        try:
            ...
            print(
                f"  오류: '{os.path.basename(path)}' 처리 중 문제 발생 - {e}. 해당 파일 건너뜜."
            )
...
if __name__ == "__main__":
    # 🚨🚨🚨 모델 경로 및 검증 데이터 경로를 실제 환경에 맞게 설정해주세요 🚨🚨🚨

    # 1. 학습된 모델 파일 경로
    # 이전 학습 스크립트에서 모델을 저장한 정확한 경로를 지정해야 합니다.
    # ==========================================================경로 지정==========================================================
    model_path = "./Wavelet/model/original_1d_txt_cnn_model.keras"
    ...
    # 4. 학습 시 사용했던 MAX_SEQUENCE_LENGTH_WAVELET 값
    # 🚨🚨🚨 모델이 예상하는 입력 길이에 맞게 220500으로 다시 수정합니다! 🚨🚨🚨
    MAX_SEQUENCE_LENGTH_WAVELET = 220500
    ...
    if val_data_file_paths and val_raw_labels:
        ...
        print(f"\n로드된 유효 검증 TXT 파일 수: {len(val_data_file_paths)}")
